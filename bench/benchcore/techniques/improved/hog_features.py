...
class HOGFeatureExtractor:
    ...
    def create_defect_template(
        self, defect_type: str, template_patches: list[np.ndarray]
    ) -> np.ndarray:
        """
    ...
    def calculate_confidence_score(self, image_patch: np.ndarray, defect_type: str) -> float:
        """
        ...
        similarity = cosine_similarity(
            patch_features.reshape(1, -1), template_features.reshape(1, -1)
        )[0, 0]
    ...
    def enhance_detection_confidence(
        ...
        for detection in detections:
            ...
            if "bbox" in detection and "label" in detection:
                ...
                if patch.size > 0:
                    ...
                    hog_confidence = self.calculate_confidence_score(patch, detection["label"])
                    ...
                    combined_confidence = 0.7 * original_confidence + 0.3 * hog_confidence
...
class DefectShapeAnalyzer:
    ...
    def analyze_defect_shape(self, mask: np.ndarray, defect_type: str) -> dict[str, float]:
        """
        ...
        shape_score = self._calculate_shape_score(defect_type, circularity, aspect_ratio, solidity)
    ...
    def _calculate_shape_score(
        self, defect_type: str, circularity: float, aspect_ratio: float, solidity: float
    ) -> float:
