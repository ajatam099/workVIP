...
class RegionGrower:
    ...
    def __init__(self, intensity_threshold: float = 10.0,
    ...
    def grow_region(self, image: np.ndarray, seed_point: Tuple[int, int],
        ...
        if (seed_x < 0 or seed_x >= width or 
            seed_y < 0 or seed_y >= height or
            mask[seed_y, seed_x] == 0):
            return grown_mask
        ...
        while queue and region_size < self.max_region_size:
            ...
            for dx, dy in directions:
                ...
                if (grown_mask[next_y, next_x] > 0 or 
                    mask[next_y, next_x] == 0):
                    continue
    ...
    def grow_from_edges(self, image: np.ndarray, edge_mask: np.ndarray,
        ...
        seed_points = list(zip(edge_points[1], edge_points[0]))  # (x, y) format
    ...
    def grow_from_detections(self, image: np.ndarray, 
                           detection_masks: List[np.ndarray],
                           object_mask: Optional[np.ndarray] = None) -> List[np.ndarray]:
        """
    ...
    def adaptive_grow_from_bbox(self, image: np.ndarray, bbox: Tuple[int, int, int, int],
                              object_mask: Optional[np.ndarray] = None) -> np.ndarray:
        """
        ...
        for seed_point in seed_points:
            ...
            seed_x, seed_y = seed_point
            if (seed_x >= 0 and seed_x < image.shape[1] and
                seed_y >= 0 and seed_y < image.shape[0]):
...
def apply_region_growing(image: np.ndarray, initial_detections: List[Tuple[int, int, int, int]],
                        object_mask: Optional[np.ndarray] = None,
                        intensity_threshold: float = 10.0) -> List[np.ndarray]:
    """
